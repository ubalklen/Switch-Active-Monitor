...
def getStridedIndexNearPoint(point, outputStride, height, width):
    return {
        "y": clamp(round(float(point["y"]) / float(outputStride)), 0, height - 1),
...
def traverseToTargetKeypoint(
    ...
    displacedPointIndices = getStridedIndexNearPoint(
        displacedPoint, outputStride, height, width
    )
...
def decodePose(root, scores, offsets, outputStride, displacementsFwd, displacementsBwd):
    ...
    for edge in reversed(range(numEdges)):
        ...
        if (
            instanceKeypoints[sourceKeypointId]["score"] > 0.0
            and math.isclose(
                instanceKeypoints[targetKeypointId]["score"], 0.0, rel_tol=1e-9
            )
            is True
        ):
